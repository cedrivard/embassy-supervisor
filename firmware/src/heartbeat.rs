...
//! resource* across a pause: the LED pin is owned by the task and kept across
//! pause→resume, never re-acquired. Pause/resume it at runtime with
//! `POST /api/control?node=heartbeat&op=pause|resume`.
//!
...
// The `heartbeat` node `static` (`Pause` mode) and the `LED` ResourceSlot are
// generated by `supervisor_graph!` in `main.rs`; this module provides the worker.
// `main` provides the LED with `LED.provide(Output::new(p.PIN_25, ..))`.
...
/// `task:` clause generates the concrete shell, and its `resources:` clause
/// threads the LED in as `&mut Output`.
/// Works on any HAL whose pin implements `StatefulOutputPin` (trait calls are
/// infallible on RP2350's `Output`, hence the `let _ =`s).
