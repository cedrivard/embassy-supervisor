...
// The `HTTP` node array, the per-index spawn glue, the worker's task shell (sized
// to the member count), the `HTTP_POOL` `ElasticPool`, and the `GRAPH.pools` entry
// are all generated by `supervisor_graph!` in `main.rs` (the `pool HTTP` entry,
// `task: http_task`: floor `Terminate` + burst `OnDemand`, all depending on `net`).
// This module just provides the plain `http_task` worker fn.
