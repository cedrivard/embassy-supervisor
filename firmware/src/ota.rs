...
/// is off. The download target is then always a literal IPv4 — [`set_target`]
/// only accepts what [`parse_ip`] accepts, and the default is an address — so a
/// real DNS lookup can never be needed. Standing in for a `DnsSocket` keeps the
/// smoltcp DNS socket implementation and its `StackResources` slot out of the
/// binary entirely (see `net::SOCKET_BUDGET`). Build with `--features dns` to
