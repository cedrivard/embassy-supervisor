...
polls). `polls/pass` below 1 is normal on RP2350 — a productive pass executes
exclusives of its own (the run-queue swap, waker RMWs), and the sticky monitor
event they post makes the very next `WFE` fall through once, so sparse wakes
settle at one trailing empty pass per productive pass (≈ 0.5 polls/pass).
Core 1's passes here are not the erratum at all: each enqueue on core 0 ends in
the pender's `SEV`, which architecturally wakes *both* cores, so an otherwise
idle core's passes/s tracks the other core's polls/s one-for-one. What matters
is the order of magnitude: see
[Note on RP2350](#note-on-rp2350) for this same report before the idle-path fix,
where the thread executor read ~1 M passes/s at 0.00 polls/pass.
...
- **`passes/s` orders of magnitude above `polls/s`** (`polls/pass` ≈ 0) — a wake
  ...
  core instead of sleeping. `busy%` will not show it anywhere (empty passes are
  booked as idle, and measured A/B the storm does not visibly slow the other
  core's polls either) — the rate gap and idle board draw are the only tells.
  See [Note on RP2350](#note-on-rp2350).
...
- **The single most diagnostic comparison is `passes/s` vs `polls/s`,** read as an
  ...
  and the regime tells you why: a productive pass's own exclusives leave a sticky
  monitor event, so the next `WFE` falls through into exactly one empty pass.
  Sparse wakes therefore settle at ≈ 0.5 (this firmware measures 0.47–0.54 near
  idle); back-to-back work absorbs the event into a pass that already has a task
  queued and approaches 1.0 (bench under self-wake saturation measures exactly
  1.00); bursty HTTP load lands between (0.55–0.63 from ~8 req/s to full `wrk`).
  `passes/s` *orders of magnitude* above `polls/s` is a **wake storm**: something
  keeps waking the executor with nothing runnable (on RP2350, see
  [below](#note-on-rp2350)). Idle board draw is the other tell. Tick-based
...
⚠️ **On RP2350, any exclusive access keeps `WFE` from sleeping.** It is a silicon quirk:
`ldaex`/`stlex` raises a global-monitor event, an effective `SEV`, and the event flag is
sticky, so the next `WFE` returns immediately. One exclusive per pass anywhere in the
executor's idle loop is enough to make the thread executor free-run at ~1 MHz. This is
not specific to this firmware or the supervisor; it affects any WFE-idle embassy or
pico-SDK program on RP2350.

Two exclusives used to sit in that loop, and this workspace removes both:

- `RunQueue::dequeue_all` called `TransferStack::take_all` (a `swap`) on every poll,
  empty or not. Fixed in embassy-executor via `cordyceps::TransferStack::is_empty`, wired
  in through the root `[patch.crates-io]` (see the sibling checkouts it names). Without
  those checkouts you get the old behaviour.
- `trace::on_poll_start` did `passes.fetch_add(1)`, which with `trace-hooks` on kept the
  loop dirty regardless. It is now a plain load/store.

Measured effect on this firmware: idle board draw with no USB and no network activity,
heartbeat only, drops from **27.65 mA to 20.35 mA**, and the pass rate collapses by five
orders of magnitude. A controlled A/B on identical firmware source — only the two
exclusives above toggled — with the same `wrk -t1 -c2 -d10s` run as
[above](#reading-a-sample-report), on core 0's thread executor:

| | passes/s | polls/s | polls/pass | busy% | in-poll% | net avg poll |
|---|---|---|---|---|---|---|
| before (storm) | 933123 | 1859 | 0.00 | 14.8 | 14.0 | 59.4 µs |
| after (fix) | 3385 | 1886 | 0.56 | 14.6 | 13.7 | 57.9 µs |

Near idle the thread executor goes from ~1.1 M passes/s to **2.5 passes/s** (heartbeat
off, no network traffic; ~22 passes/s with the heartbeat blinking and host chatter). The
watchdog node prints per-executor `Δpasses`/`Δpolls` every 2 s over RTT, so the near-idle
floor is measurable with zero network involvement — an `/api/tasks` sample is itself
network work.

Two things the storm does **not** do, both measured directly:

- **It does not slow real work.** Busy%, poll rate and `net`'s mean poll are unchanged
  across the A/B — the storm spins only inside otherwise-idle time, costing ≲ 0.2% busy.
  (An earlier revision of this table showed busy% halving and `net`'s poll dropping
  103.8 → 47.8 µs; those reports came from two different firmware revisions, and the
  effect does not reproduce on a controlled A/B. Deliberate core-1 interference doesn't
  reproduce it either: a 14 M exclusives/s storm, or a 57 M SRAM-accesses/s storm, on
  core 1 leaves core 0's poll times unchanged.)
- **It does not cross cores through the monitor.** The erratum's event is core-local:
  that 14 M exclusives/s core-1 storm wakes core 0 only ~270 times/s — once per core-1
  *scheduler pass*, which is the pender's explicit `SEV` (architecturally visible to both
  cores, one per empty→non-empty enqueue). Benign at normal rates, but it is why an
  otherwise-empty core's passes/s mirrors the other core's polls/s in every report above.

**This is a mitigation, not a fix for the erratum.** It removes only the exclusives the
executor and the supervisor emit themselves. An exclusive from your own task code, a
driver, or `embassy-time` will still defeat `WFE`, and `passes/s` far above `polls/s` is
how you will see it. When that happens, `idle %` is "WFE-spin", not power saving. For
genuine low power use the `powman` peripheral (deep sleep), not WFE.
